function [r1,r2,r3,r4,C_uobs1,C_uobs2,C_uobs3,C_uobs4,C_vobs1,C_vobs2,C_vobs3,C_vobs4,delta_obs1,delta_obs2,delta_obs3,delta_obs4]=Bin_exp(); 
    ...
    n = 200; x = linspace(-1,1,n)'; [XX,YY,ZZ] = meshgrid(x,x,x);
    X = [ XX(:) YY(:) ZZ(:) ];
    X = 2*rand(100,3)-1;      
    ...
    % delta_obs1=norm((psi1'*usub1)'.*obs1,1)/(norm((psi1'*usub1)',1));
    % delta_obs2=norm((psi1'*usub1)'.*obs2,1)/(norm((psi1'*usub1)',1));
    % delta_obs3=norm((psi1'*usub1)'.*obs3,1)/(norm((psi1'*usub1)',1));
    % delta_obs4=norm((psi1'*usub1)'.*obs4,1)/(norm((psi1'*usub1)',1));
