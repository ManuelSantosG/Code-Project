...
if Smooth==0   % without smooth scheme
    ...
    idistr=input('Select a distribution to generate wet day precipitation amount,1: Exponential, 2: Gamma, 3: Skewed normal, or 4: Mixed exp:');
...
elseif Smooth==1 % with a smooth scheme
    ...
    if idistr~=1 & idistr~=2 
        display('Note: Only exponential and gamma distributions are used to generated wet day precipitation, when smoothing')
        display('scheme is selected. However, skewed normal and mixed exponential distributions are provided, when smoothing');
        display('scheme is not selected. ');
        display('***********************************************************END**********************************************************')
        return
    end
    % the parameters are analysed at 2 weeks scale for each option of
