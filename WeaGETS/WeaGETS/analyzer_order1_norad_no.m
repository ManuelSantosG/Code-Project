function [idistr,ap00,ap10,par,A,B,aC0,aC1,aC2,aD1,aD2,sC0,sC1,sC2,sD1,...
...
if idistr == 1  % exponential distribution
    ...
    %
    for i=1:n14
        par(1,i)=sum(stot(14*(i-1)+1:14*i))/sum(sPtot(14*(i-1)+1:14*i));
    end
...
elseif idistr==3 % 3-paramter skewned normal distribution
    for i=1:n14
        P2week=PP(:,14*(i-1)+1:14*i);
        P2week=reshape(P2week,[],1);
        Pnon0=find(P2week~=0);
        par(1,i)=mean(P2week(Pnon0));
        par(2,i)=std(P2week(Pnon0));
        par(3,i)=skewness(P2week(Pnon0)); 
        % the skewned normal distribution can generate negative
        % precipitation if the skewness greater than 4.5. So it set to 4.5
        % when the observed skewness more than 4.4
        if par(3,i)>4.5
            par(3,i)=4.4;
        end
    end
    
elseif idistr == 4      %  3-parameter mixed exponential distribution
    for i=1:n14
        P2week=PP(:,14*(i-1)+1:14*i);
        P2week=reshape(P2week,[],1);
        Pnon0=find(P2week~=0);
        parmhat=mixexpfit(P2week(Pnon0));
        par(1,i)=parmhat(1);
        par(2,i)=parmhat(2);
        par(3,i)=parmhat(3);
    end
