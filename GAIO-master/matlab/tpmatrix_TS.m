function T = tpmatrix_TS(tree,f, TSeries, X, depth, verbose)
...
for k = 0:floor(N/l),                     % split in chunks of size l
    K = k*l+1:min((k+1)*l,N);
    c = b(1:d,K) ;
    r = b(d+1:2*d,1)   ;    % center and radii of the boxes
    n = size(c,2); 
    E = ones(n,1);         % sample points in all boxes
    %P = kron(E,X)*diag(r) + kron(c',ones(size(X,1),1));
    P=TSeries;
    U=tree.search(P',depth);
    pI=find(U>0); %pI are the indexes of the points IN the tree
    length(pI)
    U=U(pI);
    [B,Perm]=sort(U);
    P=P(pI,:);
    ...
    %J = kron(K',ones(size(TSeries,1),1));  % column numbers
    J=B;
